...
public class OcspValidatorCallbackHandler implements CertificateValidator {
    ...
    public static final Logger _logger = LoggerFactory.getLogger(OcspValidatorCallbackHandler.class);

//    private OxalisCertificateValidator oxalisCertificateValidator = OxalisCertificateValidator.getInstance();

    public boolean validate(X509Certificate certificate) {
        _logger.debug("Validation callback handler called: " + certificate.getSerialNumber());
        try {
            return SpringServerContext.getSimplerInvoicingCertificateValidator().validate(certificate);
        } catch (CertPathValidatorException e) {
            _logger.error("Certificate " + certificate.getSubjectDN().getName() + " failed validation; " + e.getMessage(), e);
            return false;
        }
    }
}
